...
export const TopBar: React.FC = () => {
  ...
  return (
    <header className="h-14 sm:h-16 bg-white border-b border-[#e5e7eb] px-3 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs">
      {/* Left: Mobile Drawer Trigger & Search */}
      <div className="flex items-center gap-2 sm:gap-4 flex-1 max-w-xl">
        <button
          onClick={() => setIsMobileDrawerOpen(true)}
          className="p-1.5 text-[#64748b] hover:text-[#111827] rounded-lg hover:bg-[#f1f5f9] lg:hidden transition-colors shrink-0"
          title="Open Menu"
        >
        ...
        <button
          ...
          className="w-full max-w-sm flex items-center justify-between gap-2 px-3 py-1.5 bg-[#f4f5f8] hover:bg-[#ebeef5] border border-[#e2e8f0] rounded-xl text-[#64748b] hover:text-[#191c27] text-xs transition-all group"
        >
          <div className="flex items-center gap-2 truncate">
            <Search className="w-3.5 h-3.5 text-[#94a3b8] group-hover:text-[#5a42e8] transition-colors shrink-0" />
            <span className="truncate">Search commands, records…</span>
          </div>
      ...
      {/* Right: Cloud Status, Currency, Signals Bell, Quick Add, Logout */}
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-2">
        {/* Supabase Status */}
        <button
          onClick={() => setPage('settings')}
          className="hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-[#f3f1fc] text-[#5a42e8] border border-[#e9e5f8] hover:bg-[#eae6fb] transition-colors"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold text-white bg-[#5a42e8] hover:bg-[#4a34db] shadow-xs active:scale-95 transition-all"
        >
        ...
        <button
          onClick={logout}
          className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold bg-[#fff0f2] text-[#b54b59] hover:bg-[#ffe1e5] hover:text-[#9f3744] border border-[#ffd5db] transition-colors"
